...
class SaNGreeATransformer(BaseEstimator, TransformerMixin):
    ...
    def prepareGenHierarchiesObject(self, X):
        ...
        genh_degree = CGH.CatGenHierarchy('degree', 'Exercise3/data' + os.sep + 'gen_hierarchies' + os.sep + 'Degree.json')
        genh_branch = CGH.CatGenHierarchy('branch', 'Exercise3/data' + os.sep + 'gen_hierarchies' + os.sep + 'Branch.json')
        genh_sex = CGH.CatGenHierarchy('sex', 'Exercise3/data' + os.sep + 'gen_hierarchies' + os.sep + 'Gender.json')
        genh_interest = CGH.CatGenHierarchy('interest', 'Exercise3/data' + os.sep + 'gen_hierarchies' + os.sep + 'Interest.json')
    ...
    # Initialize encoders dictionary
      ...
      self.categorical_features = set(X_knn.select_dtypes(include=["object", "category"]).columns)

      self.numeric_features = [
          col for col in X_knn.columns if col not in self.categorical_features
      ]     
    ...
    # Scaling 
      ...
      X_knn[self.numeric_features] = self._knn_scaler.fit_transform(X_knn[self.numeric_features])
...
class SaNGreeATransformer_microaggregation(BaseEstimator, TransformerMixin):
    ...
    # Scaling 
      self._knn_scaler = StandardScaler()
      X_knn[self.numeric_features] = self._knn_scaler.fit_transform(
          X_knn[self.numeric_features]
      )
