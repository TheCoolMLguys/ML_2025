import pandas as pd 
import numpy as np 
from preprocessing import breast_cancer_preprocessing, Student_placement_preprocessing, Personality_type_preprocessing, SaNGreeATransformer_microaggregation
...
def find_mean_values(metrics_list):
    ...
    for m in metrics:
        ...
        mean_results[m] = {"mean": round(np.mean(values),3), 
                           "std": round(np.std(values, ddof=1),3)}  # sample standard deviation}
...
def run(datasets, state, anonymity = True):
    ...
    for keys, data in datasets.items():
        ...
        preprocessing_class_instance = data[2](anonymity=anonymity, k=5, anonymizer_class=SaNGreeATransformer_microaggregation)  # instantiate preprocessing class
        ...
        for name, classifier in models.items():
          ...
          for train_idx, val_idx in kf.split(X_train):
            ...
            # Preprocessing (before Anonimyuzing)
            #preprocess = preprocessing_class_instance
            #cat_features = X_train_cv.select_dtypes(include=["object", "category"]).columns
            #X_train_prep = preprocess.fit_transform(X_train_cv)
            #X_val_prep   = preprocess.transform(X_val_cv)

            # Anonimiyzing (after preprocessing)
            #if anonymity:
            #   sangreea_microagg = SaNGreeATransformer_microaggregation(k=5, cat_features=cat_features)
            #   sangreea_microagg.fit(X_train_prep)
            #   X_train_anon = sangreea_microagg.transform(X_train_prep)
            #   print(X_train_anon.head(20))
            #else:
            #    X_train_anon = X_train_prep.copy()

            # new preprocessing class
            preprocess = preprocessing_class_instance
...
if __name__ == '__main__':
    ...
    df_breast_cancer = pd.read_csv("Exercise3" + os.sep + "data" + os.sep + "breast-cancer-diagnostic.shuf.lrn.csv")
    df_teen_addiction = pd.read_csv("Exercise3" + os.sep + "data"+ os.sep + "teen_phone_addiction_dataset.csv")
    df_student_placement = pd.read_csv("Exercise3" + os.sep + "data"+ os.sep + "student_placement.csv")
    ...
    print("#"*30)
    print("#"*30)
    print("Without k-Anonimity")
    run(datasets, state=42, anonymity = False)

    print("#"*30)
    print("#"*30)
    print("With k-Anonimity")
