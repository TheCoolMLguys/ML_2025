...
def run(datasets):
    ...
    for keys, data in datasets.items():
        ...
        # figure out how to add that comparison still? AKA switch from non log transform to log transform
        #if keys=='Breast_cancer':
            
            #print("\n Comparison to process with log-transformed variables:")
            #X_train_blue, y_train_blue, X_valid_blue, y_valid_blue = split_dataset(df, data[1], 0.2)

            #pipeline_blue = make_pipeline(preprocessing_class_instance, RandomForestClassifier(random_state=42))
            #y_pred_blue, train_time_blue = train_dataset(pipeline_blue, X_train_blue, y_train_blue, X_valid_blue)
            #metrics_blue = evaluate_dataset(y_valid_blue, y_pred_blue, train_time_blue)
            #print('Metrics for dataset with feature reduction:')
            #print(metrics_blue)
